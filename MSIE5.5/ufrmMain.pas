...
uses
  ...
  FileCtrl, ImgList, ActnList, Classes, System.ImageList, System.Actions;
...
Begin
  ...
  SaveUrlStart := CachBuff[52];
  Index := 0;
  Ch := CachBuff[SaveUrlStart];
  While Ch <> 0 Do
  Begin
    SaveUrl := SaveUrl + chr(Ch);
    Inc(Index);
    Ch := CachBuff[SaveUrlStart + Index];
  End;
  Index := 0;
  ...
  SaveUrl := StringReplace(SaveUrl, '//', '/', [rfReplaceAll]);
  SaveUrl := StringReplace(SaveUrl, '//', '/', [rfReplaceAll]);
  SaveUrl := StringReplace(SaveUrl, ':', '_', [rfReplaceAll]);
  SaveUrl := StringReplace(SaveUrl, '|', '_', [rfReplaceAll]);
  SaveUrl := StringReplace(SaveUrl, '?', '_', [rfReplaceAll]);
  SaveUrl := StringReplace(SaveUrl, '>', '_', [rfReplaceAll]);
  SaveUrl := StringReplace(SaveUrl, ',', '_', [rfReplaceAll]);
  ...
  { delete [n] renaming mode of msie5.x from file names }
  Bkopen := pos('[', SaveFile);
  ...
  SaveUrl := IncludeTrailingPathDelimiter(ExtractFilePath(LowerCase(SaveUrl)));
  { delete Save File from Url }
...
begin
  if SelectDirectory('MSIE Cach Directory', OpenRoot, OpenRoot) then
  Begin
    OpenRoot := IncludeTrailingBackslash(OpenRoot);
...
begin
  ...
  Begin
    SaveRoot := IncludeTrailingBackslash(SaveRoot);
...
begin
  ...
  Writeln(Log, '*******************************************************************************');
  Writeln(Log, '*                            IE5.x Cach Explorer log                          *');
  Writeln(Log, '*******************************************************************************');
  Writeln(Log);
  ...
  Writeln(Err, '*******************************************************************************');
  Writeln(Err, '*                         IE5.x Cach Explorer error log                       *');
  Writeln(Err, '*******************************************************************************');
  Writeln(Err);
  Writeln(Err, 'Log file generated on ', FormatDateTime('ddd d","mmm","yyyy hh:nnam/pm', Now), '.');
  Writeln(Err);
  Writeln(Err, 'Root Folder of MS internet Explorer 5.x Cach:');
